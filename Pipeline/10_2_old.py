...
seqOrder = []
ntOutput = open("/home/jpalme56/PycharmProjects/hiv-evolution-master/noindel3.csv", 'w')
...
for x in sorted(files):
    ...
    for i in data:
        ...
        for n, rchar in enumerate(data[i][0]):
            ...
            if rchar != "-" and qchar != "-":
                ...
                if (rchar in "ATCG") and (qchar in "ATCG"):
                    counter1[rchar] = counter1.get(rchar, 0) + 1
            ...
            # Case 3: reference sequence has an insertion
                ...
                if qTemp:
                    ...
                    # INSERTION OUTPUT: Used for generating the insertion sequences found in folder 10_1
                    # E.g. >HEADER \n >ref \n ACGTCGA,ACCAGA,AGT,
        print(rInsert)
        print(qInsert)
        ...
        for j in counter1.keys():
            ...
            if label not in noIndel.keys():
                noIndel[label] = 0
            noIndel[label] += counter1[j]

        seqOrder.append(head1)
        for k in counter2.keys():
            label = k + '.' + vregion
            if label not in noIndel.keys():
                noIndel[label] = 0
            noIndel[label] += counter2[k]
