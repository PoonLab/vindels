...
%DIF DEL /home/jpalmer/vindels/Manuscript/Phylo-Feb15/main.tex     Tue May 21 01:12:10 2019
%DIF ADD /home/jpalmer/vindels/Manuscript/Phylo-current/main.tex   Tue May 21 01:22:46 2019
...
%DIF 21a22-23
\newcommand*{\doubledag}[2]{\includegraphics[height=#2,width=#1, trim={1cm 5cm 1cm 3cm }, clip]{doubledag-symbol.pdf}} %DIF > 
\newcommand*{\singledag}[2]{\includegraphics[height=#2,width=#1, trim={1cm 5cm 1cm 3cm }, clip]{singledag-symbol.pdf}} %DIF > 
%DIF -------
...
\DIFdelbegin \DIFdel{John Palmer
}\DIFdelend \DIFaddbegin \DIFadd{Art FY Poon
}\DIFaddend 

\DIFdelbegin \DIFdel{647-867-1240
}\DIFdelend \DIFaddbegin \DIFadd{(519) 661-2111 x 87978
}\DIFaddend 

\DIFdelbegin \DIFdel{jpalme56}\DIFdelend \DIFaddbegin \DIFadd{apoon42}\DIFaddend @uwo.ca
\end{flushright}
\newpage
...
%DIF > \todo{Art}{Edited this paragraph to incorporate our focus on tree cherries }
...
Here we present results from a \DIFaddbegin \DIFadd{dated-tip }\DIFaddend phylogenetic analysis of \DIFdelbegin \DIFdel{a publicly available }\DIFdelend HIV-1 \DIFdelbegin \DIFdel{sequence database}\DIFdelend \DIFaddbegin \textit{\DIFadd{env}} \DIFadd{sequences from a public database.
By comparing sequences from different hosts, our analysis focuses on fixed indel differences that are tolerated by the virus; for example, this analysis implicitly excludes indels that induce frameshifts in }\textit{\DIFadd{env}}\DIFadd{.
Our novel phylogenetic method specifically analyzes cherries \mbox{%DIFAUXCMD
\citep{mckenzie2000distributions} }\hspace{0pt}%DIFAUXCMD
--- pairs of sequences directly descended from a common ancestor with no intervening ancestral nodes --- in time-scaled phylogenies }\DIFaddend to estimate the rates of indel evolution in the gp120 variable loops of seven HIV-1 group M subtypes and CRFs (herein referred to collectively as clades).
We \DIFaddbegin \DIFadd{focused on cherries to reduce the exposure of indels to purifying selection, and also the probability of multiple indel events occurring in the same variable region, as the divergence time in a cherry tends to be shorter on average than a random selection of two tips.
Using this method, we }\DIFaddend evaluate the hypothesis that the mean rates of indels significantly vary among the gp120 variable loops and group M clades.
...
\begin{figure}
    ...
    \caption{%
    The relationship between \DIFaddbeginFL \DIFaddFL{sequence }\DIFaddendFL root-to-tip branch lengths and \DIFdelbeginFL \DIFdelFL{associated }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{sequence }\DIFaddendFL collection dates \DIFdelbeginFL \DIFdelFL{of sequences within the }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{in }\DIFaddendFL seven \DIFaddbeginFL \DIFaddFL{clade-wise }\DIFaddendFL phylogenetic trees \DIFdelbeginFL \DIFdelFL{generated in this study}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{reconstructed from gp120 conserved region (C1-C5) alignments}\DIFaddendFL .
    \DIFdelbeginFL \DIFdelFL{The y-axis describes }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Each panel is labelled by }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{total branch length from the root to the tips of the }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{clade that its }\DIFaddendFL tree \DIFdelbeginFL \DIFdelFL{in units of expected nucleotide substitutions, while the x-axis describes the collection dates of all recorded sequences}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{represents}\DIFaddendFL . 
    %DIF > The y-axis describes the total branch lengths of sequences from the root to its tip in units of expected nucleotide substitutions, while the x-axis describes the collection dates of all sequences related by the given phylogeny. 
...
\begin{figure}[htbp]
    \centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[scale=0.37, trim={12mm 5mm 0 5mm},clip]{mosaic-combined.pdf}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[scale=0.37, trim={12mm 5mm 0 5mm},clip]{mosaic-combined3.pdf}
    \DIFaddendFL \caption{
    The distribution of indel lengths within \DIFdelbeginFL \DIFdelFL{the five gp120 variable loops }\DIFdelendFL (a) \DIFdelbeginFL \DIFdelFL{and }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL seven group M clades \DIFaddbeginFL \DIFaddFL{and }\DIFaddendFL (b) \DIFaddbeginFL \DIFaddFL{the five gp120 variable loops }\DIFaddendFL . 
    Indel lengths, measured in nucleotides, were classified into three categories: 3 nt, 6 nt, and 9 nt or longer. 
    Box heights indicate the proportion of indels belonging to the given length category, while box widths indicate the proportion of indels belonging to \DIFdelbeginFL \DIFdelFL{each variable loop }\DIFdelendFL (a) \DIFdelbeginFL \DIFdelFL{or }\DIFdelendFL each clade \DIFaddbeginFL \DIFaddFL{or }\DIFaddendFL (b) \DIFaddbeginFL \DIFaddFL{each variable loop }\DIFaddendFL .  
...
}\DIFdelend %DIF > This pattern was particularly apparent for variable loops V2 and V4.
...
\DIFdelbegin \DIFdel{We sought to reduce the impact of purifying selection by focusing on cherries in the time-scaled phylogenies for each }\DIFdelend %DIF > \todo{Art}{Added the following section in at the request of reviewers. (Considering the limitation that sequences may have indels removed from them) }
\DIFaddbegin \DIFadd{A further consideration is indels may have been removed by investigators as sequencing errors, prior to their deposition into the LANL }\DIFaddend HIV-1 \DIFdelbegin \DIFdel{clade. 
For a given tree, the divergence time between tips in a cherry will be shorter on average than a random selection of two tips, reducing the time that indel polymorphisms were exposed to selection.
This emphasis on cherries would have also reduced }\DIFdelend \DIFaddbegin \DIFadd{database, which would cause us to underestimate indel rates. 
...
%DIF > This limitation should not have a substantial impact on our study as this tends to only affect indels that are out of frame. 
%DIF > Given the hypermutation in gp120, it is common to leave all other cases of indels untouched, even those which induce a loss of function.  
%DIF > \todo{Art}{I edited the following section so that it contains different points / reads differently compared to the part I added to the introduction. Thoughts?}
%DIF > We sought to reduce the impact of purifying selection by focusing on cherries in the time-scaled phylogenies for each HIV-1 clade.
%DIF > For a given tree, the divergence time between tips in a cherry will be shorter on average than a random selection of two tips, reducing the time that indel polymorphisms were exposed to selection.
%DIF > This emphasis on cherries would have also reduced the probability of multiple indel events accumulating between the lineages in the same variable region.
We chose to focus on cherry sequences derived from between-host data as it provided phylogenetically-independent observations of indel evolution while attempting to reduce }\DIFaddend the probability of multiple \DIFdelbegin \DIFdel{indel events accumulating between the lineages in the same variable region}\DIFdelend \DIFaddbegin \DIFadd{hits and the impacts of purifying selection}\DIFaddend .  
While the comparison of within-host HIV-1 sequences would provide even shorter time scales and thereby more accurate measures of indel rates before selection, some HIV-1 subtypes and CRFs remain underrepresented in publicly available, large and longitudinal same-patient sequence data sets.
In addition, results from \citet{wood2009hiv} suggest that purifying selection against indels is relaxed in the variable regions of HIV-1 gp120.
...
We found that indel rates in subtype B gp120 were significantly lower than the reference clade \DIFaddbegin \DIFadd{01\_}\DIFaddend AE, and generally lower than the other clades in our study.
...
For instance, more frequent exposure to the \DIFaddbegin \DIFadd{vaginal }\DIFaddend mucosal layer, \textit{e.g.}, heterosexual transmission, \DIFaddbegin \DIFadd{in non-B subtypes }\DIFaddend may episodically select for reduced levels of glycosylation in the HIV-1 envelope to avoid \DIFdelbegin \DIFdel{lectin binding \mbox{%DIFAUXCMD
\citep{joseph2015bottlenecks}}\hspace{0pt}%DIFAUXCMD
.
}\DIFdelend \DIFaddbegin \DIFadd{capture by lectins, such as langerin found on vaginal Langerhans cells \mbox{%DIFAUXCMD
\citep{joseph2015bottlenecks, lederman2006microbicides, Hladik:2009}}\hspace{0pt}%DIFAUXCMD
.
%DIF > We postulate that this selective pressure may not be present to the same extent in the rectal mucosa, \textit{e.g} transmission via males who have sex with males, which is commonly exposed 
}\DIFaddend The significantly lower indel rate estimates in V3 irrespective of HIV-1 clade (Figure \ref{indel-rates}) were consistent with the functional importance of this variable loop. 
...
%DIF > Our analysis indicates that indels in V1 and V3 tend to have substantially higher frequencies of G and lower frequencies of A than the rest of the variable loop (Figure \ref{nt-plot}).
...
We then used these data to investigate how commonly indels tended to \DIFdelbegin \DIFdel{disrupt }\DIFdelend \DIFaddbegin \DIFadd{change }\DIFaddend PNGSs in the variable loops.   
%DIF > \todo{Art}{This sentence was changed}
By combining PNGS and indel location data, we searched for instances where an indel overlapped with a PNGS \DIFdelbegin \DIFdel{and induced to its loss }\DIFdelend in one of the two sequences of a cherry\DIFaddbegin \DIFadd{, indicating either the deletion of a PNGS or the insertion of a sequence containing one}\DIFaddend .
