...
means <- sapply(new.order, function (x) median(x$logged) )
means <- means[order(means)]
ordered <- names(means)
...
par(mar=c(5,5,1,1))
plot(gd.order2$subtype, gd.order2$logged, xlab="Group M Clade", ylab="log10(Genetic Distance)", cex.axis=1.2, cex.lab=1.5, varwidth=T)
